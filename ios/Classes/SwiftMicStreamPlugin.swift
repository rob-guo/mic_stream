...
enum ChannelConfig: Int {
  case CHANNEL_IN_MONO = 16
  case CHANNEL_IN_STEREO = 12
}
enum AudioSource: Int { case DEFAULT }

public class SwiftMicStreamPlugin: NSObject, FlutterStreamHandler, FlutterPlugin,
  AVCaptureAudioDataOutputSampleBufferDelegate
{
  ...
  public func onCancel(withArguments arguments: Any?, eventSink events: @escaping FlutterEventSink) -> FlutterError? {
    ...
    do {
      ...
      events(
          FlutterError(
            code: "-3",
            message: "Error stopping audio session ", details: e))
        return nil
    }
    return nil
  }
    ...
    if let audioCaptureDevice: AVCaptureDevice = AVCaptureDevice.default(for: AVMediaType.audio) {
      ...
      do {
        ...
        let preferredIOBufferDuration: TimeInterval =
          1.0 / audioSession.sampleRate * Double(self.BUFFER_SIZE)
        ...
        self.eventSink!(
          FlutterError(
            code: "-3",
            message: "Error encountered starting audio capture, see details for more information.",
            details: e))
      }
    }
  }
  ...
  public func captureOutput(
    ...
    let data = Data(
      bytesNoCopy: audioBufferList.mBuffers.mData!,
      count: Int(audioBufferList.mBuffers.mDataByteSize), deallocator: .none)
